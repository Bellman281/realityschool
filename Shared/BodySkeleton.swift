...
class BodySkeleton: Entity {
    ...
    required init(for bodyAnchor: ARBodyAnchor) {
        ...
        for jointName in ARSkeletonDefinition.defaultBody3D.jointNames {
            ...
            // Crearte an entity for the joint , and to joints directory, and add it to the parent entity
    ...
    func update(with bodyAnchor: ARBodyAnchor){
        ...
        for bone in Bones.allCases {
            ...
            //entity.look (at: skeletonBone.toJoint.position, from: skeletonBone.centerPosition,  relativeTo: nil) //set orientation of bone
            
            entity.look (at: skeletonBone.centerPosition, from: skeletonBone.toJoint.position,  relativeTo: nil) //set orientation of bone
        }
    }
    ...
    private func createSkeletonBone (bone: Bones, bodyAnchor: ARBodyAnchor) -> SkeletonBone? {
        ...
        let jointFromEntityPosition = jointFromEntityOffsetFromRoot + rootPosition // relative to world refrence

        
        
        let jointToEntityOffsetFromRoot = simd_make_float3(toJointEntityTransform.columns.3) // relative to root
        
        let jointToEntityPosition = jointToEntityOffsetFromRoot + rootPosition // relative to world refrance frame
    ...
    private func createBoneEntity (for skeletonBone: SkeletonBone, diameter: Float = 0.05 , color: UIColor = .brown) -> Entity {
